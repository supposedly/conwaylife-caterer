...
async def on_message(message):
    if message.content.startswith("!wiki"):
        ...
        data = requests.get("http://conwaylife.com/w/api.php?action=query&prop=revisions&rvprop=content&format=xml&titles=" + query).text
        if '#REDIRECT' in data:
        ...
        embed = discord.Embed(title=pgtitle, description=desc, url="http://conwaylife.com/wiki/" + query, color=0x680000)
